...
/* harmony default export */ __webpack_exports__["a"] = {
  ...
  PerceptronOld: __WEBPACK_IMPORTED_MODULE_7__classifier_perceptronold__["a" /* default */],
  SVMJS: __WEBPACK_IMPORTED_MODULE_8__classifier_svmjs__["a" /* default */],
};
 ...
 * Perceptron learner for 2 or more classes. Uses 1-vs-all classification
 */
class PerceptronOld extends __WEBPACK_IMPORTED_MODULE_0__classifier__["a" /* default */] {
  /**
   * Constructor
   *
   * @param dict optionsUser User-defined options for perceptron
  ...
  train(features, labels) {
    ...
    const converged = __WEBPACK_IMPORTED_MODULE_2__math_linalg__["a" /* default */].valueVector(false);
    let numConverged = 0;
    ...
    while (true) {
      ...
      for (const classIndex of uniqueClassIndices) {
        ...
        if (__WEBPACK_IMPORTED_MODULE_2__math_linalg__["a" /* default */].internalSum(__WEBPACK_IMPORTED_MODULE_2__math_linalg__["a" /* default */].abs(weightsIncrement)) < 0.0001) {
  ...
  predict(features) {
    ...
    // Get predictions from all classifiers for all data points by predicting all data points with
    // each classifier (getting an array of predictions for each classifier) and transposing
    const datapointsPredictions = __WEBPACK_IMPORTED_MODULE_2__math_linalg__["a" /* default */].transpose(this.classifiers.map(x => x.predict(features, { output: 'normalized' })));

    // Form final prediction by taking index of maximum normalized classifier output
    return datapointsPredictions.map(x => __WEBPACK_IMPORTED_MODULE_3__util_arrays__["a" /* default */].argMax(x));
  }
}

/* harmony default export */ __webpack_exports__["a"] = PerceptronOld;
...
/* harmony default export */ __webpack_exports__["a"] = svmjs;
